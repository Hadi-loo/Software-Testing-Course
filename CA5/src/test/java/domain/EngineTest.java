...
public class EngineTest {
    ...
    @Test
    @DisplayName("fraudulent quantity should be 0 when the order quantity is less than the average order quantity")
    public void testFraudulentQuantityIsZeroWithLessOrderQuantity() throws Exception {
        setUpWithArgs(order, 1, 5, 5, 6);
        engine.orderHistory.add(order);
        Order order2 = new Order();
        setUpWithArgs(order2, 2, 5, 5, 10);
        engine.orderHistory.add(order2);
    ...
    @Test
    @DisplayName("fraudulent quantity should be 0 when the order quantity is equal to the average order quantity")
    public void testFraudulentQuantityIsZeroWithEqualOrderQuantity() throws Exception {
        setUpWithArgs(order, 1, 5, 5, 5);
        engine.orderHistory.add(order);
        int fraudulentQuantity = engine.getCustomerFraudulentQuantity(order);
        assertEquals(0, fraudulentQuantity);
    }
